...
canonical.wide = utils::read.csv('data/execfunction.csv', header = TRUE) %>%
...
canonical.gamm = read.csv('data/feedbacklearn.csv') %>% 
...
```{r, display execfunction long 02, message = FALSE, warning = FALSE, error = FALSE}
canonical.long %>% filter(id <= 5) %>%
  kableExtra::kable(label = NA,
                    format = 'html',
                    digits = 3,
                    booktabs = TRUE,
...
Although we do not show the running and output of these models in-depth, all the **Mplus** files needed to recreate these models are available to be downloaded below.
...
We can also use the package [**foreign**]() to write out the data to a **SAS** compatible text file. The `write.foreign()` function also outputs a syntax file that imports the data into the **SAS** environment. 

```{r, write to sas 02, message = FALSE, warning = FALSE, error = FALSE}
foreign::write.foreign(canonical.long, 
                       datafile = paste0('external/',filename,'.txt'), 
                       codefile = paste0('external/import_',filename,'.sas'), 
                       package='SAS')
```

For convenience, all the **SAS** files needed to recreate these analyses are available for download below.
...
The `summary()` function output, while extensive, is not formatted for publication. However, we can use a great function `tab_model()` from the [**sjPlot**]() package to generate publication-quality tables from the MLM output. Here we will merge the results from the **nlme** and **lmerTest** packages.
...
We can move on to fitting a GAMM model. Unlike the other frameworks, we will use the `feedbacklearn` data with continuous age in this model. We could, in theory, fit the `execfunction` data, but that would largely be a waste of the GAMM framework's potential for longitudinal analysis. 

```{r canonical GAMM 01, warning = FALSE, message = FALSE}
gamm = gamm4::gamm4(modularity ~ 1 + s(age),
                    random = ~ (1 | id),
                    data = canonical.gamm)
summary(gamm$mer)
summary(gamm$gam)
...
testgamm = gamm(modularity ~ s(age), random=list(id=~1), data=canonical.gamm)
```
```{r canonical GAMM 02, warning = FALSE, message = FALSE, results='asis', eval=FALSE}
gamtabs(gamm$gam, 
        type = 'html',
        caption = 'Modularity as a Function of Age')
```
```{r canonical GAMM 03, warning = FALSE, message = FALSE}
plot(gamm$gam, se = TRUE, shade = TRUE, rug = TRUE, 
     main = 'Canonical GAMM Smooth',
     xlab = 'Age', ylab = 'Modularity')
```

```{r canonical GAMM 04, warning = FALSE, message = FALSE,  eval=FALSE}
canonical.gamm$id = factor(canonical.gamm$id)
gamm.mcgv = mgcv::gam(modularity ~ 1 + s(age) + s(id, bs='re'), 
                      data=canonical.gamm)
summary(gamm.mcgv)
coef(gamm.mcgv)
plot_smooth(gamm.mcgv, view = 'age', rm.ranef = TRUE, rug=TRUE,
            main = 'Canonical GAMM Smooth',
            xlab = 'Age', ylab = 'Modularity')
plot(gamm.mcgv)
```
...
```{r, eval=FALSE}
gamm.mcgv2 = mgcv::gam(modularity ~ 1 + s(age, k=3) + s(id, bs='re'), 
                       data=canonical.gamm)
plot_smooth(gamm.mcgv2, view = 'age', rm.ranef = TRUE, rug=TRUE,
            main = 'Canonical GAMM Smooth with 3 Basis Functions',
            xlab = 'Age', ylab = 'Modularity')

gamm.mcgv3 = mgcv::gam(modularity ~ 1 + s(age, k=20) + s(id, bs='re'), 
                       data=canonical.gamm)
plot_smooth(gamm.mcgv3, view = 'age', rm.ranef = TRUE, rug=TRUE,
            main = 'Canonical GAMM Smooth with 20 Basis Functions',
            xlab = 'Age', ylab = 'Modularity')
coef(gamm.mcgv3)
```
...
First we will define a model syntax object that specifies the model. While we will cover the basics here, consult the [**lavaan website**]() for a more complete syntax tutorial.
...
```{r canonical LCM 03}
summary(lcm, fit.measures = TRUE, estimates = TRUE, standardize = TRUE, rsquare = TRUE)
```
...
lcm = lavaan::growth(linear.lcm, 
                     ...
                     estimator = 'MLR',
                     missing = 'FIML')
summary(lcm, fit.measures = TRUE, estimates = FALSE, standardize = FALSE, rsquare = FALSE)
```
...
```{r canonical LCM 05}
summary(lcm, fit.measures = FALSE, estimates=  TRUE, standardize = TRUE, rsquare = TRUE)
```
...
We might also wish to generate a diagram visualization of the LCM model, either for model checking or for presenting results. We can use the `semPaths()` function from the [**semPlot**]() package to do just that. Here we will plot the model results including the intercepts and with black paths. The `what` argument we will set to `'est'` to scale the size of the paths to the parameter estimate.
