...
We can move on to fitting a GAMM model. Unlike the other frameworks, we will use the `feedbacklearn` data with continuous age in this model. We could, in theory, fit the `executive.function` data, but that would largely be a waste of the GAMM framework"s potential for longitudinal analysis. 
...
testgamm <- gamm(modularity ~ s(age), random=list(id=~1), data=feedback.learning)
```
...
feedback.learning$id <- factor(feedback.learning$id)
